...
def index(request):
    ...
    if request.method == 'POST':
        msg = "<p>Thank you for submitting the Blog. We will add it to our processing queue and will begin tagging and categorizing it.</p>"
        name = request.REQUEST.get("name", "")
        email = request.REQUEST.get("email", "webmaster@dizcollect.com")
        link = request.REQUEST.get("link", "")
        comment = request.REQUEST.get("comment", "")

        send_mail('Blog Rec: '+name, name + "\r\n\r\n" + comment, email,
                  [settings.CONTACT_EMAIL], fail_silently=False)


      
    return render_to_response('ursula/blogs/index.html',
        {#'items': items,
         'nav': 'blogs',
         'msg': msg,
