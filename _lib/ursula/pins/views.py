...
@login_required
def index(request):
    item_list = Item.objects.all().filter(type=1).order_by('-release_date', '-id')
    return render_to_response('ursula/pins/pin-list.html',
        {'item_list': item_list
...
def edit(request, id=""):
    ...
    if request.method == 'POST':
        if request.POST['submit_action'] == 'Add':
            ...
            if form.is_valid():
                ...
                return HttpResponseRedirect('/pins/') # Redirect after POST
            else:
                submit_action = 'Add'
    ...
    if request.method == 'GET':
        if id:
            ...
            ebay_items = Ebay_Item.objects.all().filter(item = item, is_active = 1).order_by('ebay_item_id')
        else:
            submit_action = 'Add'
        
    return render_to_response(
        'ursula/pins/pin-edit.html',
		{	'request': request,
			'form': form,
...
def search_title(request):
    ...
    item_list = Item.objects.all().filter(title = search_title, type = 1)
    return render_to_response('ursula/pins/pin-list.html',
        {'item_list': item_list
        }, context_instance=RequestContext(request)
    )    
